...
const LoginPage = () => {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      const authResponse = await axios.post(`${apiUrl}/signin`, { username, password });
      if (authResponse.status === 200) {
        // Authentication was successful, now update the "Remember Me" setting
        const { token } = authResponse.data;

        await axios.post(`${apiUrl}/set_remember_me`, { remember_me: rememberMe }, {
          headers: {
        ...
        if (rememberMe) {
          // Save token for a longer duration
          localStorage.setItem('authToken', token);
        } else {
          // Save token for a standard duration
          localStorage.setItem('authToken', token);
        }
